/**
 * 潛意識自我破壞程序測驗 — 後端接收端點（換帳號部署版，不用 Gmail 別名綁定）
 *
 * 這份檔案跟 Code.gs 的差別：Code.gs 是用 GmailApp 的「寄件地址別名」讓信件「看起來」
 * 從 pura0118@gmail.com 寄出，但實際仍是 wingxiang.work 這個帳號在執行。
 * 這份檔案則是要讓 pura0118@gmail.com 真的成為執行這支程式的帳號本身
 * （寄信配額、Apps Script 執行紀錄都會算在 pura0118 帳號上，兩個帳號完全獨立不互通）。
 *
 * 部署方式（跟 Code.gs 最大的不同在第 1、3 步）：
 * 1. 用 pura0118@gmail.com 登入 https://script.google.com，建立「新的、獨立」的專案
 *    （不要在既有的 wingxiang.work 專案裡操作，這裡要的是完全另一個專案）。
 * 2. 把本檔案內容整份貼進去。
 * 3. 上方工具列「部署」→「新增部署作業」；類型選「網頁應用程式」；
 *    「執行身分」選「我」（此時「我」＝ pura0118@gmail.com）；「誰可以存取」選「任何人」。
 * 4. 部署後複製 .../exec 網址，貼到前端 index.html 的 GAS_ENDPOINT_URL 常數，
 *    取代原本 wingxiang.work 那組網址——這一步別忘記，不然前端還是打去舊網址。
 * 5. 若之後修改本檔案，需要「管理部署作業」→ 編輯 → 新版本，才會生效。
 *
 * 重要前提：pura0118@gmail.com 必須本身就對下面的 SHEET_ID（試算表）與 FOLDER_ID
 * （雲端硬碟資料夾）有編輯權限，否則寫入試算表／上傳圖片這兩步會直接失敗。
 * 目前 pura0118 在該試算表的分享設定裡已經是「編輯者」，但雲端硬碟資料夾的權限
 * 需要另外確認——如果資料夾還沒分享給 pura0118，請先請擁有者（wingxiang.work）
 * 把 FOLDER_ID 對應的資料夾也分享給 pura0118（編輯者權限）。
 *
 * 其餘邏輯（表單解析、圖片解碼、Sheet 寫入、寄信文案）跟 Code.gs 完全一樣，
 * 只是這裡用最單純的 MailApp.sendEmail() 直接寄，不需要任何別名設定——
 * 因為執行帳號本身就是 pura0118，寄件地址自然就是 pura0118@gmail.com。
 *
...
// 這裡不需要 SENDER_EMAIL 常數——執行帳號本身就是 pura0118@gmail.com，MailApp 會自動用它寄出。
 ...
 * 寄件帳號＝目前部署這支 Apps Script 的 Google 帳號（「執行身分：我」，這份檔案預期是 pura0118@gmail.com）。
 ...
 * 這裡的「你」是指登入建立這個新專案的帳號（應該就是 pura0118@gmail.com）。
 *
 * 使用方式：把下面的 email 換成你要收測試信的信箱，選這個函式，按「執行」，
 * 完成授權後檢查該信箱是否收到測試信、寄件人是否顯示 pura0118@gmail.com。
 */
